package org.firstinspires.ftc.teamcode.FTC10158;

//TODO: update these for our robot from team 10158:
//          line 131 ROBOT_BACK_LENGTH = 10;
//          line 173 double ROBOT_FRONT_LENGTH = 10;
//          line 936  webcam label as "Webcam C270"
//      update motor mappings in FTC10158/TwoPersonDriveTeleOp
//TODO: outline of TEAM 10158's Auto program structure:
//      line 134 setBackdropGoalPose() = set locations of all target poses
//      line 161 buildPaths() = create path
//      line 262-760 autonomousPathUpdate() = process the path created--move robot.
//      line 762 setPathState(int state)
//      line 768 - 802 stackCorrection(double correctionPower) = align to stack with distance sensors
//      Line 804-856   leftDistanceSensorDisconnected(), rightDistanceSensorDisconnected() {
//                  startDistanceSensorDisconnectDetection(int state), updateDistanceSensorDisconnects()\
//      Line 894    rearDistanceSensorDisconnected()
//      Line 858 backdropCorrection(Pose scorePose, double distanceGoal) = align to backdrop with distance sensors
//      line 923 init() {
//                  foldUp = new SingleRunAction(()-> {if (Integer.parseInt(String.valueOf(pathState).substring(0,1)) < 4) setPathState(40);});
...
//                  TwoPersonDrive twoPersonDrive = new TwoPersonDrive(true);
//                  twoPersonDrive.hardwareMap = hardwareMap;
//                  twoPersonDrive.telemetry = telemetry;
//                  Timer pathTimer = new Timer();  same for opmodeTimer, scanTimer, distanceSensorUpdateTimer = new Timer(), distanceSensorDecimationTimer = new Timer();
//                  Follower follower = new Follower(hardwareMap);
//                  follower.setStartingPose(startPose);
//                  VisionPortalTeamPropPipeline teamPropPipeline = new VisionPortalTeamPropPipeline(0);
//                  VisionPortal visionPortal = new VisionPortal.Builder()
//                      .setCamera(hardwareMap.get(WebcamName.class, "Webcam C270"))
...
//                  ***why?** try {Thread.sleep(2000);} catch (InterruptedException e) {throw new RuntimeException(e);}
//                  if cases--displace status leftDistanceSensorDisconnected() rightDistanceSensorDisconnected(), rearDistanceSensorDisconnected() to display
...
//                  try {sleep(2500);} catch (InterruptedException e) {throw new RuntimeException(e);}
...
//                      telemetry.update();
//                      scanTimer.resetTimer();
//                  } else if (scanTimer.getElapsedTime() > 700) {visionPortal.setProcessorEnabled(teamPropPipeline, true);
//                  } else {visionPortal.setProcessorEnabled(teamPropPipeline, false);}
//                  }
//          line 1077: start() {
...
//                          setPathState(10);}
//          Line 900-905 loop(): updateDistanceSensorDisconnects(), follower.update();
...
//import org.firstinspires.ftc.teamcode.competition.teleop.TwoPersonDrive;            //TODO: change path as above
...
@Autonomous(name = "FTC1058 Red Close Side 2 + 4", group = "Autonomous")
public class RedRightInnerAuto_FTC10158_PedroPath_LinearOpMode extends OpMode {

    private TwoPersonDriveTeleOp twoPersonDriveTeleOp;
...
//TODO: add in value for ROBOT_BACK_LENGTH below as it is in separate constant class file in original
        double ROBOT_BACK_LENGTH = 10;
...
//TODO:add in value for ROBOT_FRONT_LENGTH below as it is in separate constant class file in original
        double ROBOT_FRONT_LENGTH = 10;
...
//TODO:delete                twoPersonDrive.moveToCustomIntakeOutPosition(INTAKE_ARM_OUT_POSITION - 0.01);
                setPathState(11);
                break;
            case 11: // detects the path to progress away from the wall and sets tangent interpolation
                if (follower.getCurrentTValue() > 0.1) {
                    ...
                    scoreSpikeMark.setLinearHeadingInterpolation(startPose.getHeading()
                            - 0.1 * MathFunctions.getTurnDirection(startPose.getHeading(),
                    scoreSpikeMark.getEndTangent().getTheta()) * MathFunctions.getSmallestAngleDifference(startPose.getHeading(),
...
//TODO: Oliver/Marcus, case 12 allow case 11 to continue to completion before going to case 13
            case 12:
                if (!follower.isBusy()) {
                    follower.holdPoint(new BezierPoint(scoreSpikeMark.getLastControlPoint()),
                            scoreSpikeMark.getEndTangent().getTheta());
                    setPathState(13);
                }
                break;

//TODO: Marcus/Oliver: when above path is done, detects for the end of the path and everything else to be in order and releases the pixel
//                     here, comment out to skip the pixel drop and test only robot move
                case 13:
...
//TODO: Marcus/Oliver:moves mechanisms into position to score and pick up from stack as well as starts moving to score
//              here, comment out on wait time for pixel drop and move arm out to move the robot and test only robot move
            case 14:
...
//TODO: Marcus/Oliver: wait time margin at the Spike Mark drop
            case 15:
...
//TODO:***** Marcus/Oliver: this move the robot to the Backdrop and do arm set up preparation
...
//                    setPathState(17);
//                }
//TODO: Marcus/Oliver: replace above with this below--move robot without outtake pixel drop at backrop action
                if (!follower.isBusy()) {
                    Follower.useHeading = true;
                    backdropGoalPoint = new Point(initialBackdropGoalPose);
                    follower.holdPoint(new BezierPoint(backdropGoalPoint), Math.PI * 1.5);
                //    twoPersonDrive.setOuttakeArmInterpolation(OUTTAKE_ARM_YELLOW_SCORE_POSITION);
                //    distanceSensorDecimationTimer.resetTimer();         //TODO: they used distance sensor?
                //    startDistanceSensorDisconnectDetection(-1);         //TODO: they used distance sensor?
                    setPathState(17);
                }
                break;

//TODO: Marcus/Oliver: case 17,18:  deactivate distance from backdrop correction, but not how they have thebackdropCorrection(initialBackdropGoalPose, 1.8) method.
            case 17:
...
//TODO: Marcus/Oliver: drop pixel at Backdrop
...
//TODO: Marcus/Oliver: pause 0.5 second (allow time to drop pixel at Backdrop)--note they add in case 110 is out of order so must have added in later.
            case 110:
                if (pathTimer.getElapsedTime() > 500) {
...
//TODO: Marcus/Oliver: It seen that they stopped and not to the stack
//                      if the Distance Sensor is off--goto Case 50--abort case--but in method: setPathState(50)
//                      Here make it to continue to first white Stack.
            ...
            case 21:
                if (!follower.isBusy()) {
                    ...
                    follower.holdPoint(new BezierPoint(new Point(firstCycleToStack.getPath(1).getLastControlPoint().getX(),
                            firstCycleToStack.getPath(1).getLastControlPoint().getY() + 1,
                            Point.CARTESIAN)),
                            firstCycleStackPose.getHeading());
                    distanceSensorDecimationTimer.resetTimer();
                    setPathState(22);
                }
                break;

//TODO: Marcus/Oliver:  if the Distance Sensor is off--goto Case 50
//      note how they use the case to do an action
//      and may be continuing to do previous action while the next is started.
            case 22:
                if (distanceSensorDisconnected) {
                    setPathState(50);
                    break;
            ...
            case 26:
                if (follower.getCurrentTValue() > 0.92) {//!follower.isBusy()) {
                    ...
                    follower.holdPoint(new BezierPoint(new Point(firstCycleStackPose)),
                            Math.PI * 1.5);
...
//TODO: Marcus/Oliver: arrive at Backdrop again
...
//TODO: Marcus/Oliver: if the Distance Sensor is off here--goto Case 211--another aborted case, else continue
            case 210:
                if (rearDistanceSensorDisconnected) {
                    setPathState(211);
                    break;
            ...
            case 30: // once the inner pixel has dropped, start the robot off to the second pass on the first stack
                if (pathTimer.getElapsedTime() > 2000) {        //TODO: change when to start
                    Follower.useHeading = true;
                    follower.resetOffset();
                    if (distanceSensorDisconnected) {
                        setPathState(50);
           ...
           //     }
            ...
            case 37: // waits for the intake claw to close and then sets the intake to move back in while pulling the extension back in slightly
               ...
               //     twoPersonDrive.setTransferState(TRANSFER_POSITIONING);
                ...
                break;
            case 38:
                //TODO: remove && twoPersonDrive.transferState == TRANSFER_PRESET_HOLD
                if (((follower.getCurrentPathNumber() == 1 && follower.getCurrentTValue() > 0.1) || !follower.isBusy()) ) {
            //        twoPersonDrive.setTransferState(TRANSFER_OUT);
                    setPathState(39);
                }
            ...
            case 39: // detects for end of the path and outtake out and drops pixel
                if (follower.atParametricEnd() ) {          //TODO: remove && twoPersonDrive.outtakeState == OUTTAKE_OUT
            ...
            case 317: // once the outer pixel has dropped, drop the inner one and fold up
                if (pathTimer.getElapsedTime() > 2000) {  //TODO: remove "> 2 * OUTTAKE_CLAW_DROP_TIME"
                    //twoPersonDrive.setTransferState(TRANSFER_RESET);
                    Follower.useHeading = true;
                    setPathState(40);
                }
...
//TODO: Marcus/Oliver:  Case 50-52 (comes from case 20):
//                      abort case at the first Backdrop when distance sensor malfunction
//                      At end, it sets setPathState(-1) in case 50
            case 50:
        //        twoPersonDrive.setTransferState(TRANSFER_RESET);
        //        twoPersonDrive.moveIntake(INTAKE_IN);
                setPathState(51);
...
*/
    ...
    public void loop() {
        ...
        follower.update();
        twoPersonDriveTeleOp.autonomousControlUpdate();
...
//        rightDistanceSensor = hardwareMap.get(DistanceSensor.class, "rightDistanceSensor");
        ...
        TwoPersonDriveTeleOp twoPersonDriveTeleOp = new TwoPersonDriveTeleOp(true);
        twoPersonDriveTeleOp.hardwareMap = hardwareMap;
        twoPersonDriveTeleOp.telemetry = telemetry;

        Timer pathTimer = new Timer();
        Timer opmodeTimer = new Timer();
        Timer scanTimer = new Timer();
        Timer distanceSensorUpdateTimer = new Timer();
        Timer distanceSensorDecimationTimer = new Timer();
        ...
        twoPersonDriveTeleOp.initialize();
       // twoPersonDrive.setIntakeArmPosition(INTAKE_ARM_IN_POSITION + 0.1);
...
//        twoPersonDrive.intakeClawIsOpen = false;
    ...
    public void start() {
        ...
        setBackdropGoalPose();
        buildPaths();
        twoPersonDriveTeleOp.frameTimer.resetTimer();
        opmodeTimer.resetTimer();
        setPathState(10);
    }
